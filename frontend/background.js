 ...
 *
 * Fail-safe policy: if the API is unreachable or times out, we return
 ...
 *
 * @param {Object} features  - The 20 features extracted by content.js / utils.js
 * @param {string} url       - The full URL of the page being checked
...
async function callPhishingAPI(features, url) {
  ...
  try {
    ...
    if (err.name === 'AbortError') {
      console.error('[PhishGuard BG] ⏱ API request timed out after', REQUEST_TIMEOUT_MS, 'ms.');
    } else {
    ...
    // Fail-safe: do not flag as phishing if backend is unavailable
...
/**
 * Listens for CHECK_URL messages sent by content.js.
 * The `return true` at the end is REQUIRED in MV3 to keep the message channel
 * open for asynchronous sendResponse calls.
 */
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  (async () => {
    ...
    // ── Persist result so popup.js can display it ─────────────────────────
...
// ── Tab cleanup ────────────────────────────────────────────────────────────────
// Remove stored scan results when a tab is closed to avoid stale data.
